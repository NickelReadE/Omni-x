...
const getLaunchpadInfo = async (): Promise<LaunchPadTypeFunc> => {
  ...
  const collectionsFeatured = collections.filter(
    (collection: LaunchPadType) => collection.mint_start_timestamp < currentTimestamp && collection.mint_end_timestamp > currentTimestamp
  );
