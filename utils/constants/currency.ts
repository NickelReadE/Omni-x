...
export const VALID_CURRENCIES: { [chain: number | string]: CurrencyType[] } = {
  [ChainIDS.BINANCE]: [getCurrency(CURRENCY_USDT, oft[ChainIDS.BINANCE], 18)],
  [ChainIDS.AVALANCHE]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.AVALANCHE], 6)],
  [ChainIDS.POLYGON]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.POLYGON], 6)],
  [ChainIDS.FANTOM]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.FANTOM], 6)],
  [ChainIDS.ETHEREUM]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.ETHEREUM], 6), getCurrency(CURRENCY_WETH, weth[ChainIDS.ETHEREUM], 18)],
  [ChainIDS.ARBITRUM]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.ARBITRUM], 6), getCurrency(CURRENCY_WETH, weth[ChainIDS.ARBITRUM], 18)],
  [ChainIDS.OPTIMISM]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.OPTIMISM], 6), getCurrency(CURRENCY_WETH, weth[ChainIDS.OPTIMISM], 18)],
  [ChainIDS.MOONBEAM]: [getCurrency(CURRENCY_USDC, usdc[ChainIDS.OPTIMISM], 6)],
  [ChainIDS.APTOS]: []
};
...
export const validateCurrencyName = (currencyName: ContractName, chainId: number) => {
  if (chainId === ChainIDS.BINANCE) {
    if (currencyName === "USDC") return "USDT";
  } else {
    if (currencyName === "USDT") return "USDC";
  }
