  ...
  useEffect(() => {
    ...
    if(localCards===null){
      if(collections.length>0){
        if(collectionsForCard.length>0){
          ...
          collections.map((item: any,index:number) => {
            slides.push(
              <CollectionCard key={index} collection={item} card={collectionsForCard.find((card: { col_url: any })=>card.col_url == item.col_url)}/>
            )
          })
        }else{
          collections.map((item: any,index:number) => {
            slides.push(
              <CollectionCard key={index} collection={item} card={null}/>
            )
          })
        }
      ...
      if(collections.length>0){
        collections.map((item: any,index:number) => {
          slides.push(
            <CollectionCard key={index} collection={item} card={collectionsForCard.length>0?collectionsForCard.find((card: { col_url: any })=>card.col_url == item.col_url):JSON.parse(localCards).find((card: { col_url: any })=>card.col_url==item.col_url)}/>
          )
        })
      }
    }
