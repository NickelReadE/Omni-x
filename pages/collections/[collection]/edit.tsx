...
const Tag = ({ active, label }: { active: boolean; label: string }) => {
  return (
    <div className={"py-1 px-2 bg-[#202020] border-[0.5px] border-secondary rounded-[4px]"}>
      <TextBodyemphasis className={`${active ? "bg-clip-text text-transparent bg-primary-gradient" : "text-primary-light"} leading-[16px]`}>
        {label}
      </TextBodyemphasis>
    </div>
  );
      ...
      <div className={"flex flex-col space-y-8 w-[450px] pt-8"}>
        ...
        <div className={"flex flex-col space-y-2"}>
          ...
          <TextBodyemphasis className={"text-secondary"}>
            you will need to confirm the transaction several times so that royalties from all supported chains are distributed to the
            address input below
          </TextBodyemphasis>
          <div className={"flex items-center justify-between space-x-4"}>
            <input
              className={"h-[36px] bg-[#202020] py-2 px-3 border border-[0.5px] border-secondary rounded-[4px] w-full"}
              placeholder={"0x"}
            />
            <input
              className={"h-[36px] bg-[#202020] py-2 px-3 border border-[0.5px] border-secondary rounded-[4px] w-[70px]"}
              placeholder={"0.00%"}
            />
          </div>
        </div>
