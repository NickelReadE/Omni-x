...
const Collection: NextPage = () => {
  ...
  const { nfts, hasMoreNFTs, fetchMoreData, refreshNfts } = useCollectionNfts(
    col_url,
    display_per_page,
    selected,
    searchObj,
  ...
  const buyComponent = () => {
    ...
    for (let i = 0; i < listNFTs.length; i++) {
      temp.push(<NFTBox nft={listNFTs[i]} key={i} col_url={col_url} onRefresh={onRefresh} />);
    }
    return temp;
  };
  ...
  return (
    <>
      <div className={classNames("w-full", "pt-6 px-0", "relative", editStyle.collection)}>
        {collectionInfo ? <CollectionBanner collection={collectionInfo} setSelectedTabIndex={setTab} /> : <SkeletonCard />}
      </div>
        ...
        <>
          <div className={"flex items-center justify-between mt-6"}>
            <div className={"flex items-center space-x-4"}>
              <Image
                src={`/images/icons/filter${filterVisible ? "_active" : ""}.svg`}
                width={38}
                height={38}
                alt={"filter"}
          ...
          <div className={`grid ${filterVisible ? "grid-cols-6" : "grid-cols-5"} gap-4 mt-6`}>
            <div className={`col-span-1 ${filterVisible ? "block" : "hidden"}`}>
              <ul className='flex flex-col space-y-2'>
                <li className='w-full'>
                  <div className={`w-full px-4 py-2 text-left text-md text-secondary font-semibold ${expandedMenu == 1 ? "active" : ""}`}>
                    Buy Now
                    <Switch
                      checked={enabled}
                      onChange={setEnabled}
                      ...
                      className={
                        "bg-transparent pull-right relative inline-flex h-[16px] w-[28px] shrink-0 cursor-pointer rounded-full border-[2px] border-[#969696] transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75 flex items-center"
                      }
                    >
                                    ...
                                    <FormControlLabel
                                      ...
                                      label={
                                        <div className='flex items-center justify-between'>
                                          <span className='font-bold text-[#4d5358]'>{valueKey}</span>
                  ...
                  >
                    <div
                      className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 ${
                        filterVisible ? "xl:grid-cols-5 2xl:grid-cols-5" : "xl:grid-cols-6 2xl:grid-cols-6"
                      } gap-4 p-1`}
                    >
                      {!isActiveBuyNow &&
                        nfts.map((item, index) => {
            ...
            <div className={"flex"}>
              ...
              >
                <button
                  type='button'
                  className={`${
                    postIndex === 0 ? "bg-transparent" : "bg-primary"
                  } w-full py-2 px-4 text-md font-medium rounded-l-lg focus:z-10 focus:ring-0`}
              ...
              >
                <button
                  type='button'
                  className={`${
                    postIndex === 1 ? "bg-transparent" : "bg-primary"
                  } w-full py-2 px-4 text-md font-medium rounded-r-md focus:z-10 focus:ring-0`}
