...
import { getBlockExplorer, getCurrencyIconByAddress, getDarkChainIconById } from "../../../utils/constants";
                  ...
                  <div className='py-4'>
                    {selectedTab === 0 && (
                      <div className={"w-full grid grid-cols-5"}>
                        <div className='col-span-1 text-secondary underline'>account</div>
                        <div className='col-span-1' />
                        <div className='col-span-1 text-secondary text-center underline'>chain</div>
                        <div className='col-span-1' />
                        <div className='col-span-1 text-secondary underline'>bid</div>
                      </div>
                    )}
                    {selectedTab === 0 &&
                          ...
                          <div key={index} onMouseEnter={() => setBidHover(index)} onMouseLeave={() => setBidHover(undefined)}>
                            <div className={"w-full grid grid-cols-5"}>
                              <Link href={`/user/${item.signer}`}>
                                <div className='col-span-1 break-all mt-3 text-lg font-bold text-secondary hover:text-blue-400 cursor-pointer'>
                                  {truncateAddress(item.signer)}
                                </div>
                              </Link>
                              <div className='col-span-1' />
                              <div className='col-span-1 flex justify-center mt-3'>
                                <img src={getDarkChainIconById(item.chain_id.toString())} className='w-[21px]' alt='icon' />
                              </div>
                              ...
                              <div className='col-span-1 mt-3 flex items-center'>
                                <img src={getCurrencyIconByAddress(item.currency)} width={21} height={21} alt='icon' className='mr-5' />
              ...
              <div className={"col-span-1"}>
                <div className={"flex justify-between items-center"}>
                  ...
                  <div className={"flex items-center space-x-4"}>
                    <div className={"flex items-center bg-[#202020] rounded-[20px] py-[5px] px-2 space-x-2"}>
                      ...
                      <TextBodyemphasis className={"text-primary-light"}>{currentNFT.likes}</TextBodyemphasis>
                    </div>
                ...
                <div className={"flex flex-row mt-4"}>
                  ...
                  <Link href={`/user/${currentNFT.owner}`}>
                    <span className={"text-primary-light text-lg ml-5 hover:underline cursor-pointer"}>
                ...
                <div className={"flex flex-col mt-4 rounded-lg w-full bg-[#202020] p-3"}>
                  ...
                  <div className={"flex items-center justify-between mt-3"}>
                    <div className={"text-primary-blue text-xxxl"}>
                      {currentNFT.price > 0 ? formatDollarAmount(currentNFT.price) : "--"}
                    </div>
                    <div className={"flex items-center space-x-4"}>
                      <SecondaryButton
                        text={"buy"}
                        className={"h-[30px]"}
                        disabled={!currentNFT.price}
                        onClick={() => {
                ...
                <Accordion title={"attributes"}>
                  <div className='w-full'>
                    {currentNFT &&
                      ...
                      currentNFT.attributes.map((attribute: any, idx: number) => {
                        ...
                        if (collection.attrs !== null && JSON.stringify(collection.attrs) !== "{}" && attrs[attribute.trait_type]) {
                          ...
                          return (
                            <div className='px-5 py-2 rounded-lg' key={idx}>
                              <p className='text-primary-green text-sm font-bold'>{attribute.trait_type}</p>
                              <div className='flex justify-start items-center mt-2'>
                                <p className='text-primary-light text-xg font-bold'>
                                  {attribute.value}
                                  <span className='ml-3 font-normal'>[{trait ? trait[1] : 0}%]</span>
                                </p>
                              </div>
                            </div>
                          );
                        ...
                        return (
                          <div className='px-5 py-2 rounded-lg' key={idx}>
                            ...
                            <div className='flex justify-start items-center mt-2'>
                              <p className='text-primary-light text-xg font-bold'>
                                {attribute.value}
                                <span className='ml-3 font-normal'>[0%]</span>
                              </p>
                            </div>
                          </div>
                        );
