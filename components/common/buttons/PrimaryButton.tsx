...
export const PrimaryButton = ({ text, loading, className, parentClassName, onClick }: IPrimaryButtonProps) => {
  ...
  return (
    <button className={`flex items-center bg-primary-gradient rounded-full p-[1px] ${parentClassName ?? ''}`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={onClick}>
      <div className={twMerge(`${hovered ? 'bg-transparent' : 'bg-primary'} flex items-center justify-center rounded-full px-[15px] py-[7px] h-full ${className ?? ''}`)}>
        {
          loading && <SpinLoader />
        }
        <div
