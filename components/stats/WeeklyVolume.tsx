...
import { TextBody, TextH3 } from "../common/Basic";
  ...
  return (
    <div className={"bg-[#202020] p-6 mt-[30px] rounded-lg"}>
      <div className={"flex items-center justify-between"}>
        <TextH3 className={"text-primary-light"}>Weekly Volume</TextH3>
        <TextBody className={"bg-primary-gradient bg-clip-text text-transparent"}>Sep 12-19</TextBody>
      </div>
          ...
          {mockData.map((data: AnalyticsData, index) => {
            return (
              <div key={index} className={"flex items-center justify-between"}>
                ...
                <div className={"ml-3 text-md font-medium text-primary-light"}>{data.value}</div>
              </div>
            );
          })}
        </div>
