      ...
      <div className={`rounded-[8px] p-[1px] ${active ? "bg-primary-gradient" : "bg-secondary"} min-w-[350px] w-full`}>
        <div className={"flex flex-col py-2 px-3 rounded-[8px] bg-primary"}>
          <div className={"flex justify-between"}>
            ...
            <div
              className={`text-xl text-shadow-sm2 ${
                active ? (comingSoon ? "text-dark-red" : "bg-clip-text text-transparent bg-primary-gradient") : "text-secondary"
              } font-medium`}
            >
              {active ? "live" : "ended"}
            </div>
          ...
          <div className={"flex justify-between items-center mt-2"}>
            <div className={"flex flex-col"}>
              <div className={"flex items-center"}>
                <TextH2
                  className={`${active ? "bg-clip-text text-transparent bg-primary-gradient" : "text-secondary"} text-shadow-sm2 mr-4`}
                >
            ...
            {whitelist_info.limit_per_wallet > 1 && (
              <div className={"flex items-center justify-between bg-[#303030] rounded-[50px] space-x-2"}>
                <img
                  src={"/images/icons/minus_circle.svg"}
                  alt={"minus"}
                ...
                <img
                  src={"/images/icons/plus_circle.svg"}
                  alt={"minus"}
                  className={"w-8 h-8 cursor-pointer"}
                  onClick={() => setQuantity(Math.min(whitelist_info.limit_per_wallet, quantity + 1))}
                />
              </div>
            )}
            {active ? (
              <PrimaryButton
                text={gasless && chainId && isSupportGelato(chainId) ? "gasless mint" : "mint"}
                className={"px-6"}
                loading={isMinting}
                parentClassName={"h-[32px]"}
                onClick={() => mint(whitelist_info.price.toString(), quantity)}
              />
            ) : (
          ...
          <TextSubtext className={"mt-2 text-primary-light text-shadow-sm2"}>
            {`${whitelist_info.max_limit > 0 ? `${whitelist_info.max_limit.toLocaleString()} max - ` : ""}${
              whitelist_info.limit_per_wallet
            } per wallet`}
          </TextSubtext>
        </div>
      </div>
