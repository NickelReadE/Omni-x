...
const Banner = ({slides, blur, menu}: BannerProps): JSX.Element => {
  ...
  const fetchNFTByAddress = async (chain: 'eth' | 'bsc' | 'polygon' | 'avalanche' | 'fantom', contractAddress: string) => {
    await timeout(1000)
    const nft = await Moralis.Web3API.account.getNFTsForContract({
      chain: chain,
  ...
  const bannerImage = useMemo(() => {
    if (cuser && cuser.banners && cuser.banners.length) {
      return process.env.API_URL + cuser.banners[0]
    }
            ...
            <div className="flex justify-between justify-center fw-60 mt-5 relative">
              <div className="bottom-[0rem] left-[4rem]  absolute">
                <Image
                  ...
                  alt="avatar"
                  onError={() => {
                    cuser.avatar && setAvatarError(true)
                  }}
                  width={200}
                  height={200}
              ...
              <div className="flex flex-col ml-[20rem] mt-[10px]">
                ...
                <div className="flex flex-row h-8">
                  <div
                    className="flex items-center text-[26px] text-slate-800 font-semibold mr-[16px]">{cuser.username ? cuser.username : 'username'}</div>
                  {
                    isGregHolder &&
