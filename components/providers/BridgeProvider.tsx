...
export const BridgeProvider = ({ children }: BridgeProviderProps): JSX.Element => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (chainId && unwrapInfo === undefined) {
        let selectedItem =
          filteredNFT.filter(
            (item: { name: string; contract_type: string }) => item.name?.startsWith("Ow") && item.contract_type === "ERC721"
          ).length > 0
            ? filteredNFT[0]
            : null;
