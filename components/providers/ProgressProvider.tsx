...
export const ProgressProvider = ({ children }: ProgressProviderProps): JSX.Element => {
  ...
  const updateLzScanStatus = () => {
    const isTxCross = (txInfo: PendingTxType) =>
      txInfo.senderChainId && txInfo.targetChainId && txInfo.senderChainId != txInfo.targetChainId;

    histories
      .filter((txInfo) => isTxCross(txInfo) && (!txInfo.lzPath || txInfo.lzPath === "https://testnet.layerzeroscan.com"))
