    ...
    if (address && chainId && !txInfo.destTxHash) {
      if (txInfo.contractType === "ERC721") {
        ...
        const eventExist = events.filter((ev) => {
          return (
            ev.args?._toAddress.toLowerCase() === address.toLowerCase() &&
  ...
  const listenBridgeEvents = async (txInfo: PendingTxType, historyIndex: number) => {
    if (address && chainId && !txInfo.destTxHash) {
      if (txInfo.contractType === "ERC721") {
        ...
        const eventExist = events.filter((ev) => {
          return ev.args?.toAddress.toLowerCase() === address?.toLowerCase() && ev.args?.tokenId.toString() === txInfo.nftItem?.token_id;
        });
        ...
        if (eventExist.length === 0) {
          noSignerOmniXInstance.on("LzReceive", async () => {
            ...
            const eventExist = events.filter((ev) => {
              return (
                ev.args?.toAddress.toLowerCase() === address?.toLowerCase() && ev.args?.tokenId.toString() === txInfo.nftItem?.token_id
              );
            });
            if (eventExist.length > 0) {
              await updateHistory(historyIndex, {
        ...
        const eventExist = events.filter((ev) => {
          return ev.args?.toAddress === address && ev.args?.tokenId.toString() === txInfo.nftItem?.token_id;
          // && ev.args?.amount.toString() === txInfo.nftItem.amount
        });

        if (eventExist.length === 0) {
          noSignerOmniX1155Instance.on("LzReceive", async () => {
            const events = await noSignerOmniX1155Instance.queryFilter(
              noSignerOmniX1155Instance.filters.LzReceive(),
              txInfo.targetBlockNumber
            );
            const eventExist = events.filter((ev) => {
              return (
                ev.args?.toAddress.toLowerCase() === address?.toLowerCase() && ev.args?.tokenId.toString() === txInfo.nftItem?.token_id
  ...
  const listenTradingNFTEvents = async (txInfo: PendingTxType, historyIndex: number, isONFTCore: boolean) => {
    if (address && txInfo.senderAddress && (!txInfo.destTxHash || !txInfo.txHash)) {
      const isTradingEvent721 = (ev: any) => {
        if (isONFTCore) {
          return (
            ev.args?.to.toLowerCase() === ethers.constants.AddressZero &&
            parseInt(ev.args?.tokenId) === parseInt(txInfo.nftItem?.token_id || "0")
          );
        }
        return (
          ev.args?.to.toLowerCase() === address.toLowerCase() && parseInt(ev.args?.tokenId) === parseInt(txInfo.nftItem?.token_id || "0")
        );
      };
      ...
      const isTradingEvent1155 = (ev: any) => {
        ...
        return (
          ev.args?.to.toLowerCase() === address.toLowerCase() &&
          parseInt(ev.args?.tokenId) === parseInt(txInfo.nftItem?.token_id || "0") &&
          ev.args?.amount === txInfo.nftItem?.amount
        );
      };
  ...
  const listenGaslessMintNFTEvents = async (txInfo: PendingTxType, historyIndex: number) => {
    if (address && txInfo.senderAddress && !txInfo.txHash) {
      const isMintEvent721 = (ev: any) => {
        return ev.args?.to.toLowerCase() === address.toLowerCase() && ev.args?.from.toLowerCase() === ethers.constants.AddressZero;
      };
