...
const BuySection: React.FC<IBuySectionProps> = ({ price, srcCurrency, currency }) => {
  ...
  return (
    <div>
      ...
      <div className='flex justify-start items-center mt-5'>
        {selectedCurrency && <img src={`/images/${selectedCurrency.icon}`} alt={"currency"} width={20} height={20} />}
        <span className={"text-primary-light text-lg mx-2"}>{price}</span>
        <span className={"text-secondary text-lg ml-2"}>~{aboutPrice}</span>
      ...
      {currency ? (
        ...
        <p className='text-primary-light text-[14px] font-light italic leading-6 w-[435px] mt-10'>
          the chosen currency is not supported on the current chain
        </p>
      )}
    </div>
  );
