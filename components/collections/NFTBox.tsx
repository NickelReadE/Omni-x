...
import { getCurrencyIconByAddress, numberLocalize, getDarkChainIconById } from "../../utils/constants";
...
const NFTBox = ({ nft, col_url, onRefresh }: IPropsNFTItem) => {
  ...
  const { isListed, highestBid, highestBidCoin, lastSale } = useOrderStatics({
    nft,
  ...
  const onTransfer = () => {
    if (chainId !== nft.chain_id) {
      return switchNetwork
        ? switchNetwork(nft.chain_id)
        : dispatch(openSnackBar({ message: "Please switch to correct network", status: "error" }));
    }
        ...
        <div className='w-full'>
          ...
          <div className={"flex flex-col justify-between min-h-[100px] p-3"}>
            <div className='flex flex-col-1 flex-row justify-between items-center'>
              <TextBodyemphasis className='text-secondary'>{nft.token_id}</TextBodyemphasis>
            ...
            {boxHovered ? (
              <div className={"flex items-center justify-between"}>
                ...
                {isOwner && <PrimaryButton text={"send"} className={"px-[23px]"} onClick={onTransfer} />}
                <div className={"w-8 h-8 flex items-center justify-center cursor-pointer"} onClick={() => setDotHover(!dotHover)}>
                  ...
                  <div className='flex items-center'>
                    {isListed && (
                      <>
                        ...
                        <TextH3 className='text-primary-light ml-2'>{numberLocalize(Number(nft?.price || 0))}</TextH3>
                      </>
                    )}
                  </div>
                </div>
            ...
            {!isOwner && isWhitelisted && (
              <>
                <div className={"flex items-center justify-between px-4 pt-2"}>
                  <div className={"text-md text-secondary font-bold"}>{nft.token_id}</div>
                  <img src={chainIcon} alt={"chainicon"} width={18} height={18} />
                </div>
                ...
                <a
                  href={`https://chat.blockscan.com/index?a=${nft.owner}`}
                  target='_blank'
                  rel='noreferrer'
