...
const ListingSection: React.FC<IListingSectionProps> = ({
  ...
  return (
    <div>
      ...
      <div className='flex justify-start items-center mt-5'>
        ...
        <div className={"relative ml-4 bg-primary-gradient p-[1px] rounded-lg h-[32px] w-[110px]"}>
          ...
          <input
            type='text'
            value={price}
            className='text-primary-light font-medium w-full h-full text-left pl-8 bg-primary rounded-lg'
            onChange={onChangePrice}
          />
        </div>
        <span className={"text-secondary text-lg ml-4"}>~{aboutPrice}</span>
      </div>
      ...
      {!!floorMessage && <p className='text-[#ADB5BD] text-[14px] font-light italic leading-6 w-[435px]'>{floorMessage}</p>}
      {showDescription && (
        <p className='text-secondary text-md font-light italic leading-6 w-[435px] mt-6'>
          *sale funds are received on the blockchain the NFT is listed on, NOT on the blockchain of the buyer
        </p>
      )}
