      ...
      <DialogTitle id="form-dialog-title" className={classes.root}>
        <div className="columns-2 mt-5">
          ...
          <div className="flex justify-end">
            <button className={`w-[132px] px-5 py-2 text-[#ADB5BD]  font-semibold text-[16px] rounded-[8px] border-2 border-[#ADB5BD] ${sellType=='fixed'?'z-10 bg-[#E9ECEF]':'bg-[#F6F8FC]'}`} onClick={() => setSellType('fixed')}>fixed price</button>
            <button className={`w-[132px] px-5 py-2 text-[#6C757D]  font-semibold text-[16px] rounded-[8px] border-2 border-[#ADB5BD] relative -left-2.5 ${sellType=='auction'?'z-10 bg-[#E9ECEF]':'bg-[#F6F8FC]'}`}>auction</button>
          </div>
        </div>
      </DialogTitle>
      <DialogContent>
