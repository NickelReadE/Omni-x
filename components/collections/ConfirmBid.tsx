...
import { CURRENCY_USDC, PERIOD_LIST } from "../../utils/constants";
...
const ConfirmBid: React.FC<IConfirmBidProps> = ({
  ...
  const [currency, setCurrency] = useState(CURRENCY_USDC);
  const [period, setPeriod] = useState(PERIOD_LIST[2]);
  const [bidStep, setStep] = useState<BidStep>(BidStep.StepBid);
  const [processing, setProcessing] = useState(false);
  const [approveTx, setApproveTx] = useState("");
  let onBidApprove: any = null,
    onBidConfirm: any = null,
    onBidDone: any = null;
  let isCollectionBid = false;
  const { onBidApprove: onTradingBidApprove, onBidConfirm: onTradingBidConfirm, onBidDone: onTradingBidDone } = useTrading(tradingInput);
  const { onCollectionBidApprove, onCollectionBidConfirm, onCollectionBidDone } = useCollectionBid(collectionBid);
