        ...
        <div className={"focus:outline-none"} onMouseEnter={() => setHovered(true)}>
          <Link href={"/account"}>
            <div className={`w-11 h-11 ${hovered ? "bg-primary-gradient" : ""} rounded-full cursor-pointer`}>
              <img src={avatarImage} alt='avatar' className='w-full h-full rounded-full' />
            </div>
          </Link>
        </div>
        <Transition
            ...
            <GradientBackground className='py-2'>
              ...
              <div className={"p-4"}>
                  ...
                  {({ open }) => (
                    <>
                      <Disclosure.Button className={`flex items-center w-full ${open ? "" : "pb-3"}`}>
                        ...
                        <span className={"grow text-left px-3 text-primary-light"}>
                          ${numberLocalize(totalUSDCBalance + totalUSDTBalance)}
                        </span>
                        <span className={"flex-none w-6"}>{open ? <DropdownArrowUp /> : <DropdownArrow />}</span>
                      </Disclosure.Button>
                          ...
                          {usdcAvailableChainIds.map((balance, index: number) => {
                            return (
                              <Tooltip tooltipContent={`${balance.balance.toLocaleString()} USDC`} key={index}>
                                <img
                                  src={getChainLogoById(balance.chainId.toString())}
                ...
                <div className={"flex items-center"}>
                  <span className={"flex-none text-primary-light w-14 italic font-bold"}>
