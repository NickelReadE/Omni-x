...
export const SelectNetworks = () => {
  ...
  return (
    <div className={'h-11'}>
      <div className={`relative inline-block text-left w-[140px] h-11 py-2 px-2 rounded-[8px] ${hovered ? 'bg-[#303030]' : 'bg-[#202020]'}`} onMouseLeave={() => setHovered(false)}>
        {
          chainId &&
              ...
              <div className={'rounded-[8px] mx-[1px] p-[1px]'}>
                {
                  SUPPORTED_CHAIN_IDS.map((chainId, index) => {
                    return (
                      <div
                        key={index}
                        className={`p-2 flex items-center space-x-2 cursor-pointer overflow-hidden ${activeIndex === index ? 'bg-[#303030]' : ''} ${index === 0 ? 'rounded-t-[8px]' : ''} ${index === SUPPORTED_CHAIN_IDS.length - 1 ? 'rounded-b-[8px]' : ''}`}
