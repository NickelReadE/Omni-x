...
const MetaMaskConnect = (): JSX.Element => {
  ...
  const signSection = () => {
    if (!isSupportChain && address) {
      return (
        <div>
          <div>
            Current network is not supported <br></br> Please switch network into Goerli
          </div>
          <div className='flex justify-center mt-12'>
            <button
              onClick={() => switchNetwork?.(CHAIN_IDS[CHAIN_TYPE.GOERLI])}
              className='rounded-[10px] border border-l-30 bg-[#B444F9] text-white p-2 hover:text-white hover:bg-[#c91812] hover:border-[#c91812] hover:fill-white focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-n-100 focus-visible:outline-none active:bg-[#990101] active:border-l-500 active:text-[#ffaa85] active:ring-0'
            >
  ...
  return (
    <>
      {show && address && (
        <div className='flex justify-center items-center w-screen h-screen bg-[#ffffff90] fixed z-[1]'>
