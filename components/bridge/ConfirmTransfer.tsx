...
const networkList = [
  ...
  SUPPORTED_CHAIN_IDS.map((chainId: number) => {
    return {
...
const ConfirmTransfer: React.FC<IConfirmTransferProps> = ({ nft, open, image, collectionName, onClose, updateModal }) => {
    ...
    try {
      if (isONFTCore) {
        ...
        if (nft.contract_type === "ERC721") {
          ...
          const estimatedFee = await lzEndpointInstance.estimateFees(
            lzTargetChainId,
          ...
          setStatus(ConfirmTransferStatus.TRANSFERRING);
          const tx = await contractInstance.wrap(
            lzTargetChainId,
  ...
  return (
    <Dialog open={open} onClose={onClose} className={"pt-4 px-10 pb-8 z-50"}>
      <div className='fixed inset-0 flex items-center justify-center p-4'>
        <div
          className={"max-w-[400px] shadow-[0_0_250px_rgba(0,0,0,1)] w-full bg-primary-gradient backdrop-blur-[10px] rounded-lg p-[1px]"}
        >
          <Dialog.Panel className={"w-full bg-[#161616] backdrop-blur-[10px] rounded-lg p-8 relative"}>
            <Dialog.Title>
                ...
                {status === undefined ? (
                  <>
                    <div className={"flex mt-5"}>
                      ...
                      <div className={"flex flex-col w-full"}>
                        <div className={"flex"}>
                          <span
                            className={`cursor-pointer border-[1px] rounded-full h-6 px-2 flex items-center justify-center text-md font-medium ${
                              target === "me" ? "text-primary-green border-primary-green" : "text-secondary border-secondary"
                            }`}
                          ...
                          <span
                            className={`cursor-pointer border-[1px] rounded-full h-6 px-2 flex items-center justify-center text-md font-medium  ${
                              target === "other" ? "text-primary-green border-primary-green" : "text-secondary border-secondary"
                            } ml-2`}
                        ...
                        {target === "other" && (
                          <div className={"flex w-full mt-3"}>
                            <input
                              className={
                                "w-full bg-transparent rounded-full text-primary-light text-md border-[1px] border-secondary pl-4 h-6"
                              }
                        ...
                        <div className={"mt-3"}>
                          <Dropdown
                            menus={networkList}
                            defaultMenu={networkList[0]}
                            className={"min-w-[140px]"}
                            onChange={(value: any) => setNetworkOption(value)}
                          />
                    ...
                    <div className='flex items-center text-primary-light text-md font-medium mt-3'>
                      ...
                      {estimatingGasFee ? (
                        ...
                        <p>
                          {estimatedFee != undefined && ethers.utils.formatEther(estimatedFee)}&nbsp;{chainInfos[senderChainId].currency}
                        </p>
                      )}
                    </div>
                  </>
                  ...
                  <>
                    <div className={`flex items-center mt-6 ${status === ConfirmTransferStatus.APPROVING ? "active" : ""}`}>
                      <p
                        className={`${
                          status === ConfirmTransferStatus.APPROVING ? "bg-primary-gradient" : "bg-secondary"
                        } w-[18px] h-[18px] rounded-full text-black text-md flex items-center justify-center font-bold`}
                      >
                        1
                      </p>
                      <p
                      ...
                      {status === ConfirmTransferStatus.APPROVING && <SpinLoader />}
                    </div>
                      ...
                      {status === ConfirmTransferStatus.APPROVING && (
                        <>
                          ...
                          <div className='tx-status-row flex items-center'>
                            ...
                            <p className='text-primary-light text-[14px] leading-[17px] ml-3'>
                              {status === ConfirmTransferStatus.APPROVING ? "confirming..." : "done"}
                            </p>
                          </div>
                    ...
                    <div className={`flex items-center mt-3 ${status === ConfirmTransferStatus.TRANSFERRING ? "active" : ""}`}>
                      <p
                        className={`${
                          status === ConfirmTransferStatus.TRANSFERRING ? "bg-primary-gradient" : "bg-secondary"
                        } w-[18px] h-[18px] rounded-full text-black text-md flex items-center justify-center font-bold`}
                      >
                        2
                      </p>
                      ...
                      {status === ConfirmTransferStatus.TRANSFERRING && <SpinLoader />}
                    </div>
